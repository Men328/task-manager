#!/usr/bin/env bash
# Smoke test end-to-end: build 2 service (mỗi service gồm gRPC server + gateway),
# chạy ở port mặc định, gọi API thật, rồi tắt.
# Yêu cầu: go, node >= 20 (dùng fetch), curl. Port 8081/8082/9081/9082 phải đang trống.
set -euo pipefail
...
cleanup() {
  kill "$IDENTITY_HTTP_PID" "$IDENTITY_GRPC_PID" "$TASK_HTTP_PID" "$TASK_GRPC_PID" 2>/dev/null || true
  wait 2>/dev/null || true
}
trap cleanup EXIT INT TERM

for _ in $(seq 1 60); do
  if curl -sf localhost:8081/healthz >/dev/null 2>&1 && curl -sf localhost:8082/healthz >/dev/null 2>&1; then
    break
  fi
  sleep 0.25
done
...
if ! curl -sf localhost:8081/healthz >/dev/null 2>&1 || ! curl -sf localhost:8082/healthz >/dev/null 2>&1; then
  ...
  tail -20 \
    ...
    "$BIN_DIR/task-grpc.log" "$BIN_DIR/task-http.log" >&2
  exit 1
fi
